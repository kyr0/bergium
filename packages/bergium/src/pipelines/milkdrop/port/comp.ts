 ...
 * (pinned revision fbac6f6). Vertex + fragment GLSL frameworks are frozen verbatim;
 * preset shader text injected via ShaderUtils. Only substitution: local `clamp` for
 * the `Math.clamp` polyfill used in generateCompColors (identical min/max).
 */

const clamp = (x: number, lo: number, hi: number): number => Math.min(hi, Math.max(lo, x));

export interface CompShaderOpts {
  mesh_width: number;
  mesh_height: number;
  texsizeX: number;
  texsizeY: number;
...
/** Per-frame MilkDrop variables read by the comp shader uniforms. */
export interface CompFrameData {
  wrap: number;
  decay: number;
  bass: number;
  mid: number;
...
export default class CompShader {
  ...
  private readonly compWidth: number;
  private readonly compHeight: number;
  ...
  private compColorVertexBuf: WebGLBuffer;
  ...
  private compColorLocation!: number;
  ...
  private gammaAdjLoc!: WebGLUniformLocation | null;
  private echoZoomLoc!: WebGLUniformLocation | null;
  private echoAlphaLoc!: WebGLUniformLocation | null;
  private echoOrientationLoc!: WebGLUniformLocation | null;
  private invertLoc!: WebGLUniformLocation | null;
  private brightenLoc!: WebGLUniformLocation | null;
  private darkenLoc!: WebGLUniformLocation | null;
  private solarizeLoc!: WebGLUniformLocation | null;
  ...
  private fShaderLoc!: WebGLUniformLocation | null;
  ...
  public constructor(gl: WebGL2RenderingContext, noise: Noise, image: ImageTextures, opts: CompShaderOpts) {
    this.gl = gl;
    this.noise = noise;
    this.image = image;
    this.rng = getRNG();
  ...
  /** Builds the fixed comp plane vertex grid + triangle indices. */
  ...
  public updateGlobals(opts: CompShaderOpts): void {
    this.mesh_width = opts.mesh_width;
    this.mesh_height = opts.mesh_height;
    this.texsizeX = opts.texsizeX;
    this.texsizeY = opts.texsizeY;
  ...
  /** Compile + link the comp program, optionally injecting preset shader text. */
  ...
  /** Recompile the comp program with new preset shader text (called on preset change). */
  ...
  /** Compute the 4-corner time-modulated hue base (4 RGB corners, 12 floats). */
  public static generateHueBase(mdVSFrame: CompFrameData): Float32Array {
  ...
  /** Generate the per-vertex RGBA comp-color grid (hue + bilinear blend alpha). */
  public generateCompColors(blending: boolean, mdVSFrame: CompFrameData, warpColor: Float32Array): Float32Array {
    ...
    for (let j = 0; j < gridY1; j++) {
      for (let i = 0; i < gridX1; i++) {
        ...
        if (blending) {
          ...
          x = clamp(x, 0, this.mesh_width - 1);
          y = clamp(y, 0, this.mesh_height - 1);
