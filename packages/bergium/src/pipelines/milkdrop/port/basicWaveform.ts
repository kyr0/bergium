 ...
 * (pinned revision fbac6f6). GLSL is frozen verbatim. Only substitution: local
 * `clamp` for the `Math.clamp` polyfill (identical min/max).
 */

const clamp = (x: number, lo: number, hi: number): number => Math.min(hi, Math.max(lo, x));

const NUM_AUDIO_SAMPLES = 512;

export interface BasicWaveformOpts {
  texsizeX: number;
  texsizeY: number;
  aspectx: number;
  aspecty: number;
...
/** Per-frame MilkDrop variables read by the basic waveform generator. */
export interface BasicWaveformData {
  wave_a: number;
  wave_scale: number;
  wave_smoothing: number;
  wave_mystery: number;
  wave_mode: number;
  old_wave_mode: number;
  wave_x: number;
  wave_y: number;
  wave_r: number;
  wave_g: number;
...
export default class BasicWaveform {
  ...
  private positions2: Float32Array;
  private oldPositions: Float32Array;
  private oldPositions2: Float32Array;
  private smoothedPositions: Float32Array;
  private smoothedPositions2: Float32Array;
  ...
  private numVert: number;
  private oldNumVert: number;
  private smoothedNumVert: number;
  private alpha: number;
  private oldAlpha: number;

  public constructor(gl: WebGL2RenderingContext, opts: BasicWaveformOpts) {
    this.gl = gl;

    this.positions = new Float32Array(NUM_AUDIO_SAMPLES * 3);
    this.positions2 = new Float32Array(NUM_AUDIO_SAMPLES * 3);
    this.oldPositions = new Float32Array(NUM_AUDIO_SAMPLES * 3);
    this.oldPositions2 = new Float32Array(NUM_AUDIO_SAMPLES * 3);
    this.smoothedPositions = new Float32Array((NUM_AUDIO_SAMPLES * 2 - 1) * 3);
    this.smoothedPositions2 = new Float32Array((NUM_AUDIO_SAMPLES * 2 - 1) * 3);
    this.color = [0, 0, 0, 1];
  ...
  public updateGlobals(opts: BasicWaveformOpts): void {
    this.texsizeX = opts.texsizeX;
    this.texsizeY = opts.texsizeY;
    this.aspectx = opts.aspectx;
    this.aspecty = opts.aspecty;
  ...
  /** Compile + link the trivial color/thick-offset shader program. */
  ...
  /** Apply scale + exponential smoothing to a raw time-domain sample array. */
  public static processWaveform(timeArray: number[], mdVSFrame: BasicWaveformData): number[] {
  ...
  /** Build the per-mode waveform vertices; returns false when nothing to draw. */
  public generateWaveform(
    blending: boolean,
    blendProgress: number,
    timeArrayL: number[],
    timeArrayR: number[],
    mdVSFrame: BasicWaveformData
  ): boolean {
