...
import AudioLevels, { type AudioLevelsSource } from "./audioLevels.js";
import blankPreset, { type BlankPreset } from "./blankPreset.js";
import PresetEquationRunner, {
  type EqVars,
  type GlobalVars,
  type RunnerOpts,
} from "./presetEquationRunner.js";
import PresetEquationRunnerWASM from "./presetEquationRunnerWASM.js";
import BasicWaveform from "./basicWaveform.js";
import CustomWaveform from "./customWaveform.js";
import CustomShape from "./customShape.js";
import Border from "./border.js";
import DarkenCenter from "./darkenCenter.js";
import MotionVectors from "./motionVectors.js";
import WarpShader, { type WarpFrameData, type WarpQData } from "./warp.js";
import CompShader, { type CompFrameData } from "./comp.js";
import OutputShader from "./output.js";
import ResampleShader from "./resample.js";
import BlurShader from "./blur.js";
import Noise from "./noise.js";
import ImageTextures from "./imageTextures.js";
import TitleText from "./titleText.js";
import BlendPattern from "./blendPattern.js";
 ...
 *
 * Mechanical TypeScript port of vendor/butterchurn/src/rendering/renderer.js
 * (pinned revision fbac2f6). Preserves numeric coercion, render/blend order,
 * ping-pong FBO swapping and the dead `this.frame` reference (see comment below).
 * The WASM/EEL branch is a frozen-asset path (plan non-goal) preserved verbatim
 * behind casts; the JS PresetEquationRunner path is the active one.
 */
...
/** Raw audio byte data passed into render() (confusingly named "audioLevels" upstream). */
export interface RenderAudioData {
...
export interface RenderOptions {
  audioLevels?: RenderAudioData | undefined;
  /** Render target framebuffer. When null (default), renders to the default framebuffer (screen). */
  target?: RenderTarget | null | undefined;
...
export interface SetRendererSizeOpts {
  meshWidth?: number;
  meshHeight?: number;
  pixelRatio?: number;
  textureRatio?: number;
...
/** Shared construction params passed to every render module. */
export interface RendererParams extends RunnerOpts {
  pixelRatio: number;
  textureRatio: number;
  texsizeX: number;
  texsizeY: number;
...
/** A butterchurn preset as the renderer sees it (warp/comp shader strings + WASM-only seams). */
export type RendererPreset = BlankPreset & {
  useWASM?: boolean;
  // WASM-only (frozen eel-wasm asset), accessed only when useWASM === true.
  globalPools?: Record<string, unknown>;
  pixel_eqs_initialize_array?: ((w: number, h: number) => void) | undefined;
  pixel_eqs_wasm?: ((...args: never[]) => void) | undefined;
  pixel_eqs_get_array?: (() => Float32Array) | undefined;
};

/** WASM-typed view of a preset equation runner (frozen EEL path only). */
interface WasmRunnerView {
  globalKeys: string[];
  qs: string[];
  runVertEQs: boolean;
  preset: RendererPreset & {
    globalPools: Record<string, Record<string, { value: number }>>;
    pixel_eqs_wasm: (...args: never[]) => void;
    pixel_eqs_get_array: () => Float32Array;
  };
}
...
export default class Renderer {
  ...
  private fps: number;
  private time: number;
  private presetTime: number;
  private lastTime: number;
  private timeHist: number[];
  private timeHistMax: number;
  private blending: boolean;
  private blendStartTime: number;
  private blendProgress: number;
  private blendDuration: number;
  // Faithful quirk: vendored source references `this.frame` which is never assigned
  // (it always evaluates to undefined; comparisons coerce to false). Kept verbatim.
  ...
  private blurRatios: number[][];

  private audioLevels: AudioLevels;

  private prevFrameBuffer: WebGLFramebuffer;
  private targetFrameBuffer: WebGLFramebuffer;
  private prevTexture: WebGLTexture;
  private targetTexture: WebGLTexture;

  private compFrameBuffer: WebGLFramebuffer;
  private compTexture: WebGLTexture;

  private anisoExt: EXT_texture_filter_anisotropic | null;

  private noise: Noise;
  private image: ImageTextures;
  private warpShader: WarpShader;
  private compShader: CompShader;
  private outputShader: OutputShader;
  private prevWarpShader: WarpShader;
  private prevCompShader: CompShader;
  private numBlurPasses: number;
  private blurShader1: BlurShader;
  private blurShader2: BlurShader;
  private blurShader3: BlurShader;
  private blurTexture1: WebGLTexture;
  private blurTexture2: WebGLTexture;
  private blurTexture3: WebGLTexture;
  private basicWaveform: BasicWaveform;
  private customWaveforms: CustomWaveform[];
  private customShapes: CustomShape[];
  private prevCustomWaveforms: CustomWaveform[];
  private prevCustomShapes: CustomShape[];
  private darkenCenter: DarkenCenter;
  private innerBorder: Border;
  private outerBorder: Border;
  private motionVectors: MotionVectors;
  private titleText: TitleText;
  private blendPattern: BlendPattern;
  private resampleShader: ResampleShader;

  private supertext: { startTime: number; duration?: number };

  private warpUVs: Float32Array;
  private warpColor: Float32Array;
  /** Phase 2: render target framebuffer. When null, renders to default framebuffer. */
  private renderTarget: RenderTarget | null = null;

  private blankPreset: RendererPreset;

  private preset: RendererPreset;
  private prevPreset: RendererPreset;
  private presetEquationRunner: PresetEquationRunner;
  private prevPresetEquationRunner: PresetEquationRunner;
  private regVars!: EqVars;

  // Cached for re-render (set during render()).
  private globalVars!: GlobalVars & EqVars;
  private mdVSFrame!: EqVars;
  private mdVSFrameMixed!: EqVars;
  private prevMDVSFrame!: EqVars;
  private mdVSVertex!: EqVars;

  public constructor(gl: WebGL2RenderingContext, audio: AudioProcessor, opts: RendererOpts = {}) {
    this.gl = gl;
    this.audio = audio;
    ...
    this.audioLevels = new AudioLevels(this.audio as unknown as AudioLevelsSource);
    ...
    const globalVars: GlobalVars & EqVars = {
      frame: 0,
      time: 0,
      fps: 45,
      bass: 1,
  ...
  /** Highest blur sampler referenced in a preset's shader text (0..3). */
  public static getHighestBlur(t: string): number {
  ...
  public loadPreset(preset: RendererPreset, blendTime: number): void {
    ...
    if (this.preset.useWASM) {
      // WASM path (frozen eel-wasm asset). Preserved verbatim behind casts.
      const wasmPreset = this.preset as unknown as {
        globalPools: { perFrame: { old_wave_mode: { value: number } } };
        baseVals: EqVars;
      };
      wasmPreset.globalPools.perFrame.old_wave_mode.value = num(this.prevPreset.baseVals.wave_mode);
      this.preset.baseVals.old_wave_mode = num(this.prevPreset.baseVals.wave_mode);
  ...
  public setRendererSize(width: number, height: number, opts: SetRendererSizeOpts): void {
  ...
  public setInternalMeshSize(width: number, height: number): void {
  ...
  public setOutputAA(useAA: boolean): void {
  ...
  public calcTimeAndFPS(elapsedTime?: number): void {
    let elapsed: number;
    if (elapsedTime) {
      elapsed = elapsedTime;
    } else {
  ...
  public runPixelEquations(
    presetEquationRunner: PresetEquationRunner,
    mdVSFrame: EqVars,
    globalVars: GlobalVars & EqVars,
    blending: boolean
  ): void {
    ...
    const warpTimeV = this.time * num(mdVSFrame.warpanimspeed);
    const warpScaleInv = 1.0 / num(mdVSFrame.warpscale);
    ...
    let offset = 0;
    let offsetColor = 0;
    if (!(presetEquationRunner.preset as unknown as { useWASM?: boolean }).useWASM) {
      let mdVSVertex: EqVars = Utils.cloneVars(mdVSFrame);

      let warp = num(mdVSVertex.warp);
      let zoom = num(mdVSVertex.zoom);
      let zoomExp = num(mdVSVertex.zoomexp);
      let cx = num(mdVSVertex.cx);
      let cy = num(mdVSVertex.cy);
      let sx = num(mdVSVertex.sx);
      let sy = num(mdVSVertex.sy);
      let dx = num(mdVSVertex.dx);
      let dy = num(mdVSVertex.dy);
      let rot = num(mdVSVertex.rot);
      ...
      for (let iz = 0; iz < gridZ1; iz++) {
        for (let ix = 0; ix < gridX1; ix++) {
          ...
          if (presetEquationRunner.runVertEQs) {
            ...
            warp = num(mdVSVertex.warp);
            zoom = num(mdVSVertex.zoom);
            zoomExp = num(mdVSVertex.zoomexp);
            cx = num(mdVSVertex.cx);
            cy = num(mdVSVertex.cy);
            sx = num(mdVSVertex.sx);
            sy = num(mdVSVertex.sy);
            dx = num(mdVSVertex.dx);
            dy = num(mdVSVertex.dy);
            rot = num(mdVSVertex.rot);
          }
      ...
      // WASM path (frozen eel-wasm asset). Preserved verbatim behind casts.
      const wasmRunner = presetEquationRunner as unknown as WasmRunnerView;
      const varPool = wasmRunner.preset.globalPools.perVertex!;

      Utils.setWasm(varPool, globalVars as unknown as Record<string, number>, wasmRunner.globalKeys);
      Utils.setWasm(
        varPool,
        presetEquationRunner.mdVSQAfterFrame as unknown as Record<string, number>,
        wasmRunner.qs
      );

      varPool.zoom!.value = num(mdVSFrame.zoom);
      varPool.zoomexp!.value = num(mdVSFrame.zoomexp);
      varPool.rot!.value = num(mdVSFrame.rot);
      varPool.warp!.value = num(mdVSFrame.warp);
      varPool.cx!.value = num(mdVSFrame.cx);
      varPool.cy!.value = num(mdVSFrame.cy);
      varPool.dx!.value = num(mdVSFrame.dx);
      varPool.dy!.value = num(mdVSFrame.dy);
      varPool.sx!.value = num(mdVSFrame.sx);
      varPool.sy!.value = num(mdVSFrame.sy);

      wasmRunner.preset.pixel_eqs_wasm(
        wasmRunner.runVertEQs as never,
        this.mesh_width as never,
        this.mesh_height as never,
        this.time as never,
        mdVSFrame.warpanimspeed as never,
        mdVSFrame.warpscale as never,
        this.aspectx as never,
        this.aspecty as never
      );

      if (!blending) {
        this.warpUVs = wasmRunner.preset.pixel_eqs_get_array();
        this.warpColor.fill(1);
      } else {
        const newWarpUVs = wasmRunner.preset.pixel_eqs_get_array();

        let offsetW = 0;
        let offsetColorW = 0;
        for (let iz = 0; iz < gridZ1; iz++) {
          for (let ix = 0; ix < gridX1; ix++) {
            const u = newWarpUVs[offsetW]!;
            const v = newWarpUVs[offsetW + 1]!;

            let mix2 =
              this.blendPattern.vertInfoA[offsetW / 2]! * this.blendProgress +
              this.blendPattern.vertInfoC[offsetW / 2]!;
            mix2 = clamp(mix2, 0, 1);

            this.warpUVs[offsetW] = this.warpUVs[offsetW]! * mix2 + u * (1 - mix2);
            this.warpUVs[offsetW + 1] =
              this.warpUVs[offsetW + 1]! * mix2 + v * (1 - mix2);

            this.warpColor[offsetColorW + 0] = 1;
            this.warpColor[offsetColorW + 1] = 1;
            this.warpColor[offsetColorW + 2] = 1;
            this.warpColor[offsetColorW + 3] = mix2;
  ...
  /** Blend two frames' per-frame equation outputs across blendProgress. */
  public static mixFrameEquations(blendProgress: number, mdVSFrame: EqVars, mdVSFramePrev: EqVars): EqVars {
    const mix = 0.5 - 0.5 * Math.cos(blendProgress * Math.PI);
    const mix2 = 1 - mix;
    const snapPoint = 0.5;

    const mixedFrame: EqVars = Utils.cloneVars(mdVSFrame);

    mixedFrame.decay = mix * num(mdVSFrame.decay) + mix2 * num(mdVSFramePrev.decay);
    mixedFrame.wave_a = mix * num(mdVSFrame.wave_a) + mix2 * num(mdVSFramePrev.wave_a);
    mixedFrame.wave_r = mix * num(mdVSFrame.wave_r) + mix2 * num(mdVSFramePrev.wave_r);
    mixedFrame.wave_g = mix * num(mdVSFrame.wave_g) + mix2 * num(mdVSFramePrev.wave_g);
    mixedFrame.wave_b = mix * num(mdVSFrame.wave_b) + mix2 * num(mdVSFramePrev.wave_b);
    mixedFrame.wave_x = mix * num(mdVSFrame.wave_x) + mix2 * num(mdVSFramePrev.wave_x);
    mixedFrame.wave_y = mix * num(mdVSFrame.wave_y) + mix2 * num(mdVSFramePrev.wave_y);
    mixedFrame.wave_mystery =
      mix * num(mdVSFrame.wave_mystery) + mix2 * num(mdVSFramePrev.wave_mystery);
    mixedFrame.ob_size = mix * num(mdVSFrame.ob_size) + mix2 * num(mdVSFramePrev.ob_size);
    mixedFrame.ob_r = mix * num(mdVSFrame.ob_r) + mix2 * num(mdVSFramePrev.ob_r);
    mixedFrame.ob_g = mix * num(mdVSFrame.ob_g) + mix2 * num(mdVSFramePrev.ob_g);
    mixedFrame.ob_b = mix * num(mdVSFrame.ob_b) + mix2 * num(mdVSFramePrev.ob_b);
    mixedFrame.ob_a = mix * num(mdVSFrame.ob_a) + mix2 * num(mdVSFramePrev.ob_a);
    mixedFrame.ib_size = mix * num(mdVSFrame.ib_size) + mix2 * num(mdVSFramePrev.ib_size);
    mixedFrame.ib_r = mix * num(mdVSFrame.ib_r) + mix2 * num(mdVSFramePrev.ib_r);
    mixedFrame.ib_g = mix * num(mdVSFrame.ib_g) + mix2 * num(mdVSFramePrev.ib_g);
    mixedFrame.ib_b = mix * num(mdVSFrame.ib_b) + mix2 * num(mdVSFramePrev.ib_b);
    mixedFrame.ib_a = mix * num(mdVSFrame.ib_a) + mix2 * num(mdVSFramePrev.ib_a);
    mixedFrame.mv_x = mix * num(mdVSFrame.mv_x) + mix2 * num(mdVSFramePrev.mv_x);
    mixedFrame.mv_y = mix * num(mdVSFrame.mv_y) + mix2 * num(mdVSFramePrev.mv_y);
    mixedFrame.mv_dx = mix * num(mdVSFrame.mv_dx) + mix2 * num(mdVSFramePrev.mv_dx);
    mixedFrame.mv_dy = mix * num(mdVSFrame.mv_dy) + mix2 * num(mdVSFramePrev.mv_dy);
    mixedFrame.mv_l = mix * num(mdVSFrame.mv_l) + mix2 * num(mdVSFramePrev.mv_l);
    mixedFrame.mv_r = mix * num(mdVSFrame.mv_r) + mix2 * num(mdVSFramePrev.mv_r);
    mixedFrame.mv_g = mix * num(mdVSFrame.mv_g) + mix2 * num(mdVSFramePrev.mv_g);
    mixedFrame.mv_b = mix * num(mdVSFrame.mv_b) + mix2 * num(mdVSFramePrev.mv_b);
    mixedFrame.mv_a = mix * num(mdVSFrame.mv_a) + mix2 * num(mdVSFramePrev.mv_a);
    mixedFrame.echo_zoom =
      mix * num(mdVSFrame.echo_zoom) + mix2 * num(mdVSFramePrev.echo_zoom);
    mixedFrame.echo_alpha =
      mix * num(mdVSFrame.echo_alpha) + mix2 * num(mdVSFramePrev.echo_alpha);
    mixedFrame.echo_orient =
      mix * num(mdVSFrame.echo_orient) + mix2 * num(mdVSFramePrev.echo_orient);
    ...
    mixedFrame.b1n = mix * num(mdVSFrame.b1n) + mix2 * num(mdVSFramePrev.b1n);
    mixedFrame.b2n = mix * num(mdVSFrame.b2n) + mix2 * num(mdVSFramePrev.b2n);
    mixedFrame.b3n = mix * num(mdVSFrame.b3n) + mix2 * num(mdVSFramePrev.b3n);
    mixedFrame.b1x = mix * num(mdVSFrame.b1x) + mix2 * num(mdVSFramePrev.b1x);
    mixedFrame.b2x = mix * num(mdVSFrame.b2x) + mix2 * num(mdVSFramePrev.b2x);
    mixedFrame.b3x = mix * num(mdVSFrame.b3x) + mix2 * num(mdVSFramePrev.b3x);
    mixedFrame.b1ed = mix * num(mdVSFrame.b1ed) + mix2 * num(mdVSFramePrev.b1ed);

    return mixedFrame;
  }

  /** Derive the 3 blur min/max ranges from a frame's blur-value fields. */
  public static getBlurValues(mdVSFrame: EqVars): { blurMins: number[]; blurMaxs: number[] } {
    let blurMin1 = num(mdVSFrame.b1n);
    let blurMin2 = num(mdVSFrame.b2n);
    let blurMin3 = num(mdVSFrame.b3n);
    let blurMax1 = num(mdVSFrame.b1x);
    let blurMax2 = num(mdVSFrame.b2x);
    let blurMax3 = num(mdVSFrame.b3x);
  ...
  public bindFrambufferAndSetViewport(fb: WebGLFramebuffer | null, width: number, height: number): void {
    this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, fb);
    this.gl.viewport(0, 0, width, height);
  }

  public bindFrameBufferTexture(targetFrameBuffer: WebGLFramebuffer, targetTexture: WebGLTexture): void {
  ...
  public render({ audioLevels, elapsedTime, target }: RenderOptions = {}): void {
    // Phase 2: store the compositor-supplied render target for renderToScreen()
    ...
    const prevGlobalVars: GlobalVars & EqVars = Object.assign({}, globalVars);
    ...
    if (!this.blending) {
      this.warpShader.renderQuadTexture(
        ...
        blurMins,
        blurMaxs,
        mdVSFrame as unknown as WarpFrameData,
        this.presetEquationRunner.mdVSQAfterFrame as unknown as WarpQData,
        this.warpUVs,
        this.warpColor
      );
    } else {
      this.prevWarpShader.renderQuadTexture(
        ...
        blurMins,
        blurMaxs,
        this.prevMDVSFrame as unknown as WarpFrameData,
        this.prevPresetEquationRunner.mdVSQAfterFrame as unknown as WarpQData,
        this.warpUVs,
        this.warpColor
      );
      ...
      this.warpShader.renderQuadTexture(
        ...
        blurMins,
        blurMaxs,
        mdVSFrameMixed as unknown as WarpFrameData,
        this.presetEquationRunner.mdVSQAfterFrame as unknown as WarpQData,
        this.warpUVs,
        this.warpColor
      );
    ...
    if (this.preset.waves && this.preset.waves.length > 0) {
      this.customWaveforms.forEach((waveform, i) => {
        waveform.drawCustomWaveform(
          this.blending ? this.blendProgress : 1,
          this.audio.timeArrayL as unknown as Float32Array,
          this.audio.timeArrayR as unknown as Float32Array,
    ...
    this.basicWaveform.drawBasicWaveform(
      this.blending,
      this.blendProgress,
      this.audio.timeArrayL as unknown as number[],
      this.audio.timeArrayR as unknown as number[],
      mdVSFrameMixed as unknown as import("./basicWaveform.js").BasicWaveformData
    );

    this.darkenCenter.drawDarkenCenter(mdVSFrameMixed);

    const outerColor = [
      num(mdVSFrameMixed.ob_r),
      num(mdVSFrameMixed.ob_g),
      num(mdVSFrameMixed.ob_b),
      num(mdVSFrameMixed.ob_a),
    ];
    this.outerBorder.drawBorder(outerColor, num(mdVSFrameMixed.ob_size), 0);

    const innerColor = [
      num(mdVSFrameMixed.ib_r),
      num(mdVSFrameMixed.ib_g),
      num(mdVSFrameMixed.ib_b),
      num(mdVSFrameMixed.ib_a),
    ];
    this.innerBorder.drawBorder(
      innerColor,
      num(mdVSFrameMixed.ib_size),
      num(mdVSFrameMixed.ob_size)
    );
  ...
  /**
   * Present the composite buffer to the default framebuffer (screen) or to a
   * supplied `RenderTarget` (Phase 2: compositor integration).
   *
   * When `this.renderTarget` is set (Phase 2 path), binds that target's
   * framebuffer and renders at the target's dimensions. When null (legacy path),
   * binds the default framebuffer (screen) at canvas dimensions.
   */
  public renderToScreen(): void {
    ...
    if (this.outputFXAA) {
      ...
      // Phase 2: render to the Compositor-supplied RenderTarget
      this.bindFrambufferAndSetViewport(
        (target as unknown as { framebuffer: WebGLFramebuffer }).framebuffer,
        target.descriptor.width,
        target.descriptor.height
      );
    } else {
      // Legacy: render to the default framebuffer (screen)
    ...
    if (!this.blending) {
      this.compShader.renderQuadTexture(
        ...
        blurMins,
        blurMaxs,
        this.mdVSFrame as unknown as CompFrameData,
        this.presetEquationRunner.mdVSQAfterFrame as unknown as WarpQData,
        this.warpColor
      );
    } else {
      this.prevCompShader.renderQuadTexture(
        ...
        blurMins,
        blurMaxs,
        this.prevMDVSFrame as unknown as CompFrameData,
        this.prevPresetEquationRunner.mdVSQAfterFrame as unknown as WarpQData,
        this.warpColor
      );
      ...
      this.compShader.renderQuadTexture(
        ...
        blurMins,
        blurMaxs,
        this.mdVSFrameMixed as unknown as CompFrameData,
        this.presetEquationRunner.mdVSQAfterFrame as unknown as WarpQData,
        this.warpColor
      );
    }
  ...
  public launchSongTitleAnim(text: string): void {
    this.supertext = {
      startTime: this.time,
      duration: 1.7,
    };
  ...
  public toDataURL(): string {
    ...
    // flip data
    Array.from({ length: this.texsizeY }, (_val, i) =>
