 ...
 * output in tests/oracle/milkdrop-port-audio.test.ts. Drives Butterchurn's
 * spectrum (bass/mid/treble) from the time-domain audio window.
 */

type Wave = ArrayLike<number>;

export default class FFT {
  public samplesIn: number;
  public samplesOut: number;
  public equalize: boolean;
  public NFREQ: number;
  public equalizeArr: Float32Array | undefined;
  public bitrevtable!: Uint16Array;
  public cossintable!: [Float32Array, Float32Array];

  public constructor(samplesIn: number, samplesOut: number, equalize = false) {
  ...
  public timeToFrequencyDomain(waveDataIn: Wave): Float32Array {
    ...
    for (let i = 0; i < this.NFREQ; i++) {
      ...
      if (idx < this.samplesIn) {
        real[i] = waveDataIn[idx] as number;
      } else {
        real[i] = 0;
      }
      imag[i] = 0;
